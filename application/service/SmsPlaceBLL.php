...
class SmsPlaceBLL extends BLL {
  ...
  function setSign($place, $signId) {
    // 1.签名不存在或者不可用,报错
    $sign = model('sms')->getInfo(['logicId'=>$signId,'type'=>'sign']);
    if($sign === null || $sign['status']==='pending' || $sign['status']==='fail') {
      thrower('sms', 'signNotFound');
    }
    // 2.签名设为使用中,不可被删除
    if($sign['status'] === 'success') {
      model('sms')->edit(['logicId'=>$signId], ['status'=>'using']);
    }
    // 3.更改占位短信的签名
    ...
    if($oldId !==0 && $oldId !== $signId) {
      ...
      if($one === null) {
        model('sms')->edit(['logicId'=>$oldId], ['status'=>'success']);
      }
    }
    return true;
  }
